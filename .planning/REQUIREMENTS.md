...
- [ ] **ASSET-01**: Plugin detects complex compositions (nested groups/vectors with high child count, masks, blend modes) and flags them for image export
- [ ] **ASSET-02**: Plugin exports detected complex compositions as single PNG images instead of describing their individual parts
- [ ] **ASSET-03**: Brief maps each exported asset to its exact position in the layout tree via breadcrumb paths (e.g., "Hero > Header > Icon")
- [ ] **ASSET-04**: Asset-to-layout mapping uses nodeId as stable key to prevent filename/path misalignment
